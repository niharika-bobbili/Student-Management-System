...
def connect_database():
    def connect():
        try:
            conn = pymysql.connect(host='localhost', user='root', password='niharika@777')
        ...
        try:
            ...
            mycursor.execute(query)
            query = 'create table student(Roll_no varchar(30) PRIMARY KEY, name VARCHAR(30), Mobile_no VARCHAR(10), email VARCHAR(30),Address VARCHAR(100),gender VARCHAR(6), Dob VARCHAR(30), addedDate VARCHAR(30), addedTime VARCHAR(30))'
            mycursor.execute(query)
        except:
            query = 'USE StudentData'
            mycursor.execute(query)
